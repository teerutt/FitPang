...
Future<List<String>> listTables(Database database) async {
  ...
  final result = await database.rawQuery("SELECT name FROM sqlite_master WHERE type='table'");
...
class User {
  ...
  User({this.user_id, required this.f_name, required this.l_name, required this.email, required this.password, required this.DOB});
...
Future<void> insertUser(User user) async {
  ...
  try{
    ...
    await db.insert('user_account', user.toMap(),
